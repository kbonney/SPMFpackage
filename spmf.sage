...
#    Misc   #
#############
# defining the commonly used A[S] notation
def bracket(A,S):
    return S.transpose()*A*S
# a helpful method to turn a matrix into a binary quadratic form
def matToBQF(A):
    if A[0,1] != A[1,0]:
        pass
        #raise error 
...
# used to multiply two multivariate power series
def BQFsplitter(B):
    a=B[0]
    b=B[1]
    c=B[2]
    ...
    return D
# function for finding elements of the multiplicative group modulo n
...
# this function calculates Eisenstein coeffs "from scratch"
def makeCoeff(B,k):
    if k <= 0:
        raise Exception("Weight must be a positive even integer")
    if type(B) != BinaryQF:
        raise Exception("Binary quadratic form must be of class BinaryQF")
    ...
    z1 = zeta(1 - k)
    z2 = zeta(3 - (2 * k))
    #make a list of nonzero coefficients of our BQF to find a proper gcd (1)
    list = []
    for i in [a, b, c]:
        if i != 0:
            list.append(i)       
    sm = 0
    #iterate through divisors of our BQF (this forms the core sum) this bit along with (1) is messy, could use cleaning
...
# The core Siegel paramodular form class
class SPMF: 
    def __init__(self, k):
        if type(k)==sage.rings.integer.Integer:
            self.weight = k
            self.type = 'eisen'
    ...
    def coeff(self,B):
        ...
        if self.type == 'dict':
            B = B.reduced_form()
            cf = "(" + str(B[0])+ ", " + str(B[1]) + ", " + str(B[2]) + ")"
            if str(-B.discriminant()) in self.coeffdict["Fourier_coefficients"] and \
                cf in self.coeffdict["Fourier_coefficients"][str(-B.discriminant())]:
                return int(self.coeffdict["Fourier_coefficients"][str(-B.discriminant())][cf])
            else:
                raise Exception("Insufficient coefficient data provided")
        if self.type == 'mult':
            coeff = 0
        ...
        if self.type == 'twist':
            ...
            b = B[1]
            c = B[2]
            #this needs to be changed once we implement level, we really should be checking if the level
            #divides a, not just p**4. this works for a form twisted from level 1
            if a % p**4 != 0:
                return 0
            #case (i)
...
# current working method for operations on SPMF. better understanding of class structure/methods will probably
# reveal better approaches
